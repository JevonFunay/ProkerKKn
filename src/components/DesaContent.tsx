...
const DusunMap = dynamic(() => import("@/components/DusunMap"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 text-sm">
      Memuat peta…
    </div>
  ),
});
...
const infoCards = [
  ...
  },
  {
    ...
    value: "69 KK",
    color: "bg-teal-50 text-teal-700 dark:bg-teal-500/15 dark:text-teal-400",
  },
  {
    icon: Users,
    label: "Jumlah Jiwa",
  ...
  },
  {
    ...
    color: "bg-orange-50 text-orange-700 dark:bg-orange-500/15 dark:text-orange-400",
  },
];
              ...
              {infoCards.map((card) => {
                ...
                return (
                  <div key={card.label} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">{card.label}</p>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100 leading-snug">{card.value}</p>
                  </div>
                );
              })}
            </motion.div>
          ...
          <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-20">
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <Navigation size={13} className="text-primary-600 dark:text-primary-400" />
                <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Peta Lokasi</span>
              </div>

              <div className="w-full rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm relative z-0" style={{ height: 420 }}>
                <DusunMap />
              </div>
          ...
          <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {infoSections.map((section, i) => (
                ...
                >
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white mb-3">{section.title}</h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">{section.content}</p>
                </motion.div>
              ))}
            </div>
          </div>
